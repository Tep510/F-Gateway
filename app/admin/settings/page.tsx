...
export default function AdminSettings() {
  ...
  // サービスアカウントメールアドレス（Friendslogi Google Workspace）
  ...
  // Load Google Drive settings when tab changes to google
  useEffect(() => {
    if (activeTab === "google" && status === "authenticated") {
      loadDriveSettings()
    }
  }, [activeTab, status])
          ...
          {tabs.map(tab => (
            <button
              ...
              className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${
                activeTab === tab.key
                  ? "border-white dark:border-white text-gray-900 dark:text-white"
