...
// Fallback CSV column mapping (Japanese header -> field name)
// Used when no ClientProductColumnMapping is configured
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!file || !clientIdStr) {
      await log.warn('product_import', 'validation_error', 'ファイルまたはクライアントIDが不足', {
        requestId,
    ...
    // Log import start
    await log.info('product_import', 'import_start', `商品マスタインポート開始: ${file.name}`, {
      requestId,
      clientId,
      userId: session.user.id,
      metadata: {
    ...
    if (savedMapping?.isConfigured) {
      // Use saved column mappings (column index based)
    ...
    if (!('productCode' in columnIndices)) {
      return NextResponse.json(
        { error: savedMapping?.isConfigured
          ? 'カラムマッピングで商品コードが設定されていません'
          : '商品コード column not found in CSV' },
        { status: 400 }
      )
    }

    // Check for janCode if using saved mapping (it's required)
    if (savedMapping?.isConfigured && !('janCode' in columnIndices)) {
      return NextResponse.json(
        { error: 'カラムマッピングでJANコードが設定されていません' },
        { status: 400 }
      )
    }
    ...
    // Update import log
    const finalStatus = errorRows > 0 && insertedRows + updatedRows === 0 ? 'failed' : 'completed'
    ...
    await log.info('product_import', 'import_complete', `商品マスタインポート完了: ${file.name}`, {
      requestId,
      clientId,
      userId: session.user.id,
      durationMs,
    ...
    // Log error
    await log.error('product_import', 'import_error', '商品マスタインポートエラー', err, {
      requestId,
      durationMs,
    })
