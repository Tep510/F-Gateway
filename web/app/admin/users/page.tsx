            ...
            <div className="flex gap-3 items-end flex-wrap">
              <div>
                ...
                <input
                  ...
                  className={`px-3 py-2 border border-gray-300 rounded text-sm w-64 focus:outline-none focus:ring-2 focus:ring-blue-500 ${editingUser ? 'bg-gray-100' : ''}`}
                />
              </div>
              <div>
              ...
              <div>
                <label className="block text-sm text-gray-600 mb-1">紐付けクライアント</label>
                <select
                  value={formData.clientId}
                  onChange={e => setFormData(prev => ({ ...prev, clientId: e.target.value }))}
                  className="px-3 py-2 border border-gray-300 rounded text-sm w-56 focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200">
                  ...
                  <th className="text-left py-3 px-4 text-sm font-medium text-gray-700">紐付けクライアント</th>
              ...
              <tbody>
                {users.map(user => (
                  <tr key={user.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4 text-sm">
                      {user.client ? (
                        <span className="text-gray-900">{user.client.clientCode} - {user.client.clientName}</span>
                      ) : (
                    ...
                    <td className="py-3 px-4 text-sm space-x-2">
                      <button onClick={() => openEditForm(user)} className="text-blue-600 hover:text-blue-800 text-sm">編集</button>
                      <button onClick={() => handleDelete(user.id)} className="text-red-600 hover:text-red-800 text-sm">削除</button>
                    </td>
                  </tr>
                ))}
                {users.length === 0 && (
                  <tr><td colSpan={7} className="py-8 text-center text-gray-400 text-sm">ユーザーがいません</td></tr>
                )}
              </tbody>
            </table>
          </div>
